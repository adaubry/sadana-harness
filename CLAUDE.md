...
- A turn's or child turn's stable prompt is read from the conversation itself (`system_prompt[:stable_prompt_len]`), never from process-wide state — a conversation's voice is fixed at creation, and any writer of `system_prompt` must preserve `system_prompt[:stable_prompt_len]`
...
- Signal (1) arrived in H16, so `conversation_store.py`'s connection question is settled and not reopened: the console puts several principals on one box, which is the second long-lived in-process writer the old bullet was watching for. The shape is one writer connection per process behind a module-level `RLock` that `write_txn` takes **inside** a transaction and never around a model round trip; thread-local readers from `stores.Connections.reader()`; and one `threading.Lock` per conversation from `stores.conversation_lock(key)`, held for a whole turn. hermes's refcounted per-path registry stays declined — it lets many independently-constructed store objects share a connection, and we construct exactly one (`stores.Connections`), so it would be the registry-of-one this file already forbids. Signal (2) is still open and unchanged: a seam for a second storage backend belongs at construction, as a Protocol, only once a second real backend is being built.
- A SQLite transaction belongs to a **connection**, not a thread: a read issued on the writer while another thread is inside `write_txn` runs inside that thread's transaction and sees rows it may roll back. A read on any path that can run concurrently goes through `stores.Connections.reader()`; `runtime.conn` is for writes. Per-conversation locking does not cover this — it partitions `conversations`, while `memory_entries` and `persona_selections` are shared across every conversation an account has.
- A thread-local SQLite reader connection is never also held in a process-lifetime collection: the thread-local is the only reference, so the connection dies with its thread. A set kept "so they can be closed" is the descriptor leak `hermes_state.py:4818-4845` names; the upgrade path is a bounded pool, never a strong registry.
...
- An inbound channel envelope never carries the account it belongs to — the account is derived from the envelope by the bridge, or stated by a trusted in-process caller; a value parsed from a payload is never one
...
- A test's precondition check reads state the test itself created, never real global host state (a systemd unit path, a real port, a real file outside the test's own tmp dir) — a prior, unrelated real action can make that precondition false on a host nothing else about the test controls.
