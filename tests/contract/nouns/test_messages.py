...
def test_create_succeeds_and_returns_the_user_message(keys, tmp_path: Path, monkeypatch, schema: dict) -> None:
    monkeypatch.setattr(model_access, "send", lambda request: plain_response("hi there"))
...
def test_a_failed_turn_with_no_completion_leaves_no_assistant_row(keys, tmp_path: Path, monkeypatch) -> None:
    ...
    listed = handle(req("GET", f"/v1/conversations/{conversation_key}/messages", token=token), ctx=ctx)
    data = json.loads(listed.body)["data"]
    # Only the user row exists -- `conversation.py`'s own turn loop never
    # appended an assistant one, since no completion ever returned.
    assert len(data) == 1
